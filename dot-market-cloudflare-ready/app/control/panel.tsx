...
/**
 * Everything used to sit in one column, so finding the store settings meant
 * scrolling past every shop and every review. The areas are unrelated to each
 * other, so they are separate screens now.
 *
 * All of them stay mounted and the inactive ones are hidden. Half-typed prices
 * and unsaved shop settings live in component state, and losing those on a tab
 * change would be a worse problem than the one this solves.
 */
type Tab = "ledger" | "stats" | "shops" | "store" | "reviews" | "site";

export default function ControlPanel({
  initialShops, initialReviews, initialApplications, initialFeatureRanks, dmInviteUrl,
  storeItems, storePurchases, storeReviews, storeChannelId, storeGuildId, licenceServer,
  popup, stats,
}: {
  initialShops: Shop[];
  ...
  storeReviews: ModeratedStoreReview[];
  storeChannelId: string;
  storeGuildId: string;
  licenceServer: { url: string; hasToken: boolean };
  popup: Popup;
  /** Null when the numbers could not be read; the rest of the page still works. */
  stats: Stats | null;
}) {
  ...
  const waitingApplications = applications.filter((application) => !application.handled).length;
  const hiddenReviews = reviews.filter((review) => review.hidden).length;

  const TABS: { id: Tab; label: string; badge: string }[] = [
    { id: "ledger", label: "장부", badge: "" },
    { id: "stats", label: "숫자판", badge: "" },
    { id: "shops", label: "샵", badge: waitingApplications > 0 ? String(waitingApplications) : "" },
    { id: "store", label: "상점", badge: "" },
    { id: "reviews", label: "후기", badge: hiddenReviews > 0 ? String(hiddenReviews) : "" },
    { id: "site", label: "사이트", badge: "" },
  ];

  /** Jumping to the create form is no use if the form is on another screen. */
  const startFromApplication = (application: Application) => {
    setTab("shops");
    fillFromApplication(application);
  };

  return (
    <main className="control-page">
      <header>
        <Link href="/admin">← 샵 관리</Link>
        <strong>DOT MARKET CONTROL</strong>
        <Link href="/">마켓 보기</Link>
      </header>

      <nav className="control-tabs" aria-label="관리 영역">
        {TABS.map((entry) => (
          <button
            key={entry.id}
            type="button"
      ...
      <section className="control-shell">
        ...
        <div className="control-tab" hidden={tab !== "ledger"}>
          <LedgerPanel say={say} busy={busy} setBusy={setBusy}/>
        </div>

        <div className="control-tab" hidden={tab !== "stats"}>
          {stats
            ? <StatsPanel initialStats={stats} initialPeriod={7}/>
            : <p className="field-help">숫자를 읽지 못했습니다. 새로고침해 주세요.</p>}
        </div>

        <div className="control-tab" hidden={tab !== "shops"}>
          <form className="create-shop-card" onSubmit={create}>
            <div>
              <h2>새 샵 만들기</h2>
              <p>별도 관리자 비밀번호는 만들지 않습니다. 지정된 이메일로 로그인한 사람만 해당 샵을 관리합니다.</p>
            </div>
            <div className="create-fields">
              <label>샵 이름
                <input required value={form.name} placeholder="예: 루나 도트 공방"
                  onChange={(e) => setForm({ ...form, name: e.target.value })}/>
              </label>
              <label>샵 주소
                <input required pattern="[a-z0-9]+(?:-[a-z0-9]+)*" value={form.slug} placeholder="luna-pixel"
                  onChange={(e) => setForm({ ...form, slug: e.target.value.toLowerCase() })}/>
                <small>/shop/{form.slug || "luna-pixel"}</small>
              </label>
              <label>샵 관리자 로그인 이메일
                <input required type="email" value={form.managerEmail} placeholder="manager@example.com"
                  onChange={(e) => setForm({ ...form, managerEmail: e.target.value })}/>
                <small>이 주소로 Cloudflare Access 로그인 코드를 받습니다</small>
              </label>
              <label className="wide">샵 소개
                <textarea rows={2} value={form.description}
                  onChange={(e) => setForm({ ...form, description: e.target.value })}/>
              </label>
            </div>
            <button disabled={busy}>{busy ? "처리 중…" : "샵 생성하기"}</button>
          </form>

          <div className="control-list-head">
            <h2>입점 신청</h2>
            <span>대기 {waitingApplications}건 · 전체 {applications.length}건</span>
          </div>
          <div className="application-list">
            {applications.length === 0 ? (
              <p className="field-help">아직 들어온 신청이 없습니다. 마켓 샵 목록 끝의 빈 칸을 눌러 신청할 수 있습니다.</p>
            ) : applications.map((application) => (
              <article key={application.id} className={application.handled ? "handled" : ""}>
                <div className="application-head">
                  <b>{application.shopName}</b>
                  {application.wantedSlug && <code>/shop/{application.wantedSlug}</code>}
                  {application.handled ? <em>처리함</em> : <i>대기 중</i>}
                </div>
                <div className="application-who">
                  <McName nick={application.mcNick || application.applicantName}/>
                  {(application.affiliation || application.job) && (
                    <small>{[application.affiliation, application.job].filter(Boolean).join(" · ")}</small>
                  )}
                  <code>{application.email}</code>
                  <code>{application.applicantName} · {application.applicantId}</code>
                  <time dateTime={application.createdAt}>{application.createdAt.slice(0, 10)}</time>
                </div>
                {application.intro && <p>{application.intro}</p>}
                {application.note && <p className="application-note">{application.note}</p>}
                <div className="application-actions">
                  <button type="button" onClick={() => startFromApplication(application)} disabled={busy}>
                    이 내용으로 샵 만들기
                  </button>
                  <button type="button" onClick={() => markApplication(application, !application.handled)} disabled={busy}>
                    {application.handled ? "대기로 되돌리기" : "처리함으로"}
                  </button>
                  <button type="button" className="danger" onClick={() => removeApplication(application)} disabled={busy}>
                    삭제
                  </button>
                </div>
              </article>
          ...
          <div className="control-list-head">
            <h2>전체 샵</h2>
            <span>{shops.length}개</span>
          </div>
          <div className="control-shop-list">
            {shops.map((shop) => {
              const draft = drafts[shop.id];
              return (
                <article key={shop.id}>
                  <div className="control-shop-title">
                    <div><h3>{shop.name}</h3><span>/shop/{shop.slug}</span></div>
                    <i className={shop.webhookConfigured ? "ready" : "pending"}>
                      {shop.webhookConfigured ? "웹훅 연결됨" : "웹훅 미연결"}
                    </i>
                  </div>
                  <label>샵 관리자 로그인 이메일
                    <input type="email" value={draft?.managerEmail ?? ""}
                      onChange={(e) => setDrafts({ ...drafts, [shop.id]: { ...draft, managerEmail: e.target.value } })}/>
                    <small>Access에도 이 주소가 허용돼 있어야 로그인됩니다</small>
                  </label>
                  <label className="switch-row">
                    <input type="checkbox" checked={draft?.active ?? false}
                      onChange={(e) => setDrafts({ ...drafts, [shop.id]: { ...draft, active: e.target.checked } })}/>
                    <span>마켓과 주문 화면에 공개</span>
                  </label>
                  <label className="switch-row">
                    <input type="checkbox" checked={draft?.premium ?? false}
                      onChange={(e) => setDrafts({ ...drafts, [shop.id]: { ...draft, premium: e.target.checked } })}/>
                    <span>프리미엄 표시</span>
                  </label>
                  <label className="rank-row">추천순 고정 위치
                    <input type="number" min={0} max={999} value={draft?.featureRank ?? 0}
                      onChange={(e) => setDrafts({ ...drafts, [shop.id]: {
                        ...draft, featureRank: Math.max(0, Math.min(999, Math.trunc(Number(e.target.value) || 0))),
                      } })}/>
                    <small>1이 맨 앞. 0이면 평점·후기대로 자동 정렬됩니다. 이 값은 총괄만 보이고 어디에도 표시되지 않습니다.</small>
                  </label>
                  <div className={`control-shop-foot${isDirty(shop) ? " dirty" : ""}`}>
                    <Link href={`/shop/${shop.slug}`}>샵 보기 ↗</Link>
                    <span className="control-save-state">{isDirty(shop) ? "저장하지 않은 변경사항" : "저장됨"}</span>
                    <div className="control-shop-actions">
                      <button type="button" className="danger" onClick={() => remove(shop)} disabled={busy}>샵 삭제</button>
                      <button type="button" onClick={() => update(shop)} disabled={busy || !isDirty(shop)}>
                        {isDirty(shop) ? "변경 저장" : "저장됨"}
                      </button>
                    </div>
                  </div>
                </article>
        ...
        {/* One mount, two screens: the store's own state has to survive the move
            between them, and its reviews belong with the other reviews. */}
        <div className="control-tab" hidden={tab !== "store" && tab !== "reviews"}>
          <StorePanel
            show={tab === "reviews" ? "reviews" : "store"}
            initialItems={storeItems} initialPurchases={storePurchases} initialReviews={storeReviews}
            initialChannelId={storeChannelId} initialGuildId={storeGuildId} licenceServer={licenceServer}
            say={say} busy={busy} setBusy={setBusy}/>
        </div>

        <div className="control-tab" hidden={tab !== "reviews"}>
          <div className="control-list-head">
            <h2>그림샵 후기</h2>
            <span>{reviews.length}개 · 숨김 {hiddenReviews}개</span>
          </div>
          <div className="moderation-list">
            {reviews.length === 0 ? <p className="field-help">아직 후기가 없습니다.</p> : reviews.map((review) => (
              <article key={review.id} className={review.hidden ? "hidden-review" : ""}>
                <div className="moderation-head">
                  <b>{"★".repeat(review.rating)}<i>{"★".repeat(5 - review.rating)}</i></b>
                  <span>{review.displayName}</span>
                  <small>{review.shopName}</small>
                ...
                <div className="moderation-actions">
                  ...
                  <button type="button" className="danger" onClick={() => purge(review)} disabled={busy}>
                    완전 삭제
                  </button>
                </div>
              </article>
        ...
        <div className="control-tab" hidden={tab !== "site"}>
          <PopupPanel initialPopup={popup} say={say} busy={busy} setBusy={setBusy}/>
          <section className="dm-reach">
            <h2>손님에게 디스코드 알림이 가게 하려면</h2>
            <p>봇은 <b>같은 서버에 있는 사람에게만</b> DM을 보낼 수 있습니다. 지금은 각 샵의 서버에만 들어가 있어서, 그 서버에 없는 손님은 수락·거절·완성 알림을 DM으로 받지 못합니다. (내 주문 화면에서는 항상 보입니다.)</p>
            <p>도스온라인 본서버에 봇을 넣어두면 그 서버에 있는 사람 전원에게 DM이 갑니다. 아래 링크는 <b>권한을 하나도 요구하지 않습니다</b> — 채널을 읽지도, 글을 쓰지도 못하고, 그저 서버에 있기만 합니다.</p>
            {dmInviteUrl
              ? <a className="plain-upload-button" href={dmInviteUrl} target="_blank" rel="noreferrer">권한 없이 서버에 초대하기 ↗</a>
              : <p className="field-help">디스코드 설정이 아직 되어 있지 않습니다.</p>}
            <p className="field-help">서버에 봇을 추가하려면 그 서버에서 &quot;서버 관리&quot; 권한이 필요합니다. 없으면 서버 주인에게 이 링크를 전달해 주세요.</p>
          </section>
        </div>

      </section>
      {message && <div className={`control-message ${messageKind}`}>{message}</div>}
    </main>
  );
}
