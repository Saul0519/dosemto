...
export default function ControlPanel({ initialShops, initialReviews }: {
  ...
  const toggleReview = async (review: ModeratedReview) => {
    ...
    try {
      ...
      setMessage(review.hidden ? "후기를 다시 보이게 했습니다." : "후기를 숨겼습니다. 샵 페이지에는 숨긴 개수가 표시됩니다.");
...
`
    ...
    if (typed === null) return;
    if (typed.trim() !== review.orderId) { setMessage("입력한 주문번호가 달라서 지우지 않았습니다."); return; }
    setBusy(true); setMessage("");
    try {
